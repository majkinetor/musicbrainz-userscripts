...
(function () {
  ...
  // The shared design tokens (#562). Definitions live in dev/design-tokens.mjs
  // and are inlined here by dev/sync-tokens.mjs — edit them THERE, never here.
  ...
  function ensureLauncher() {
    ...
    launcher.style.cssText = 'position:fixed;right:14px;bottom:14px;z-index:2147483646;width:40px;height:40px;border-radius:50%;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;background:color-mix(in srgb, var(--mbu-bg) 55%, transparent);color:var(--mbu-info);box-shadow:0 2px 8px rgba(0,0,0,.18);transition:background .15s,transform .1s;opacity:.55';
  ...
  function ensurePanel() {
    ...
    panel.innerHTML = `
      ...
      <div id="falcon-body-log" style="display:none;padding:0;overflow:hidden;flex:1;flex-direction:column">
        <div class="falcon-bar" style="display:flex;align-items:center;gap:8px;padding:6px 10px;border-bottom:1px solid var(--mbu-border);font-size:11px;color:var(--mbu-text);flex:0 0 auto">
          ...
          <span id="falcon-log-copied" style="color:var(--mbu-ok)"></span>
        </div>
  ...
  function renderRowDetail(it) {
    ...
    const urlRows = it.urls.map(u => {
      ...
      return `<div style="display:flex;align-items:center;gap:6px;padding:2px 0 2px 30px;font-size:10.5px;overflow:hidden" title="${res && res.error ? esc(res.error) : ''}">
        ...
        <a href="${esc(u.url)}" target="_blank" rel="noopener" style="color:var(--mbu-info);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:0 1 auto">${esc(u.url)}</a>
        ${u.linkTypeId ? `<span style="color:var(--mbu-text-weak);flex:0 0 auto">type ${esc(u.linkTypeId)}</span>` : ''}
      </div>`;
    }).join('');
    ...
    const metaDisambig = canDisambig ? `
      <div style="display:flex;align-items:center;gap:6px;padding:3px 0 3px 30px;font-size:10.5px">
        ...
        ${it.entityType === 'recording' ? `<input type="text" class="falcon-isrc-input" data-id="${it.id}" placeholder="ISRC(s), comma-separated" value="${esc((it.isrcs || []).join(', '))}" ${it.status === 'active' ? 'disabled' : ''}
          style="flex:1 1 auto;min-width:0;font-size:10.5px;padding:2px 6px;border:1px solid var(--mbu-border);border-radius:3px;font-family:'Courier New',monospace" />` : ''}
        ${it.entityType === 'recording' ? `<label title="Flag this recording as a video (MusicBrainz's Video checkbox)" style="flex:0 0 auto;display:flex;align-items:center;gap:4px;cursor:pointer;white-space:nowrap;color:var(--mbu-text-dim)">
    ...
    const aliasRow = `
      <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;padding:3px 0 3px 30px;font-size:10.5px">
        <span style="color:var(--mbu-text-dim);flex:0 0 auto">🏷 aliases</span>
        ${aliasList || '<span style="color:var(--mbu-text-weak);font-style:italic">none</span>'}
        <input type="text" class="falcon-alias-add" data-id="${it.id}" placeholder="add alias — name, or name@locale" ${it.status === 'active' ? 'disabled' : ''}
          style="flex:1 1 140px;min-width:110px;font-size:10.5px;padding:2px 6px;border:1px solid var(--mbu-border);border-radius:3px" />
      </div>`;
