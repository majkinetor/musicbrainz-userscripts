...
(function () {
  ...
  style.textContent = MBU_TOKENS + MBU_UI_CSS + `
    ...
    /* A persistent line so the handle is discoverable, not just a hover surprise
       (#471 review) — except it was drawn in --mbu-bg-sunken, which is a SURFACE
       token: 94% of the background mixed with a little ink, i.e. ~#f0f0f0 on a
       white header. Invisible, exactly as majkinetor reported ("column
       separators are not visible on light theme, one visible is the one I hover
       over"). --mbu-border is the token for a line you are meant to see, and is
       what this table's own medium rows already use. */
    .ii-col-resize::after { content: ''; position: absolute; top: 4px; bottom: 4px; left: 3px; width: 1px; background: var(--mbu-border); }
  ...
  /* #587 (majkinetor: "Dedup it."). The overview page fetches the release once on
     load to prime the button's missing-ISRC count, and opening the panel fetched
     it AGAIN — `if (!RELEASE)` is still true while the first one is in flight, so
     a click during that window started a second identical request. Two calls for
     the same data on every open.

     Resolved, not re-requested: an already-loaded release is handed back as-is,
     and a call made while one is in flight joins it. The in-flight handle is
     cleared on settle so a FAILED load can still be retried. */
  let _relInFlight = null;
  function fetchRelease() {
    if (RELEASE) return Promise.resolve(RELEASE);
    if (_relInFlight) return _relInFlight;
    _relInFlight = fetchReleaseNow();
    _relInFlight.catch(() => {}).then(() => { _relInFlight = null; });
    // #607: the page button's count follows EVERY successful load, not just the
    // page-load one. That one can fail (MB 503s under its rate limit) and leave
    // "?" — and opening the dialog later loaded the release without telling it.
    _relInFlight.then(() => { try { updateBtnStatus(); } catch (e) {} }, () => {});
    return _relInFlight;
  }
  function fetchReleaseNow() {
    return gmGet(
  ...
  function updateBtnStatus() {
    ...
    if (missing === 0) {
      ...
      btn.title = '';   // drop a "could not load" tooltip left by a failed first load (#607)
    } else {
