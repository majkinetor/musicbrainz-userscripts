...
(function () {
  ...
  function editNoteSig() {
    ...
    // Same shape as every other script in the repo: "<Name> v<version> by <author> - <url>".
    // GT used to put the author before the version, which is the odd one out and made
    // version stats unparseable for anything reading edit notes (dev/script-metrics #570).
    return `${name} v${s.version || VERSION} by ${s.author || 'majkinetor'} - ${s.homepageURL || s.homepage || GT_HOMEPAGE}`;
  }
  ...
  function injectStyle() {
    ...
    s.textContent = MBU_TOKENS + MBU_UI_CSS + `
      ...
      /* #564 (majkinetor, light theme): "black on blue name highlight". The two
         highlights shared one blue background and were told apart by their TEXT
         colour, and --mbu-warn on that blue computes to about 1.1:1 — the amber
         and the blue have nearly the same luminance, so the name vanished into
         its own highlight. (Custom-highlight styling can't be read back with
         getComputedStyle, which is why it survived: it has to be measured off
         rendered pixels, in both engines — see test/verify-564-highlight-readable.mjs.)
         The distinction now lives in the BACKGROUND, with white text on both.
         Literal colours, not tokens: the pair has to stay legible on its own
         terms and neither background follows the page theme. */
      ::highlight(gt-hl-existing){background:#1f6feb;color:#fff}
      ::highlight(gt-hl-new){background:#b45309;color:#fff}
      ...
      .gt-tip .gt-tip-stat{color:#a9bed4;font-size:11px;margin-top:1px}   /* #564: the pill is always dark, so this is too — --mbu-info is mixed toward the page text and went near-black on it in the light theme */
  ...
  // #613: the exact-identity rule now comes from the shared matching module
  // (dev/match/artist-match.mjs, inlined below by dev/match/sync-match.mjs); the txp*
  // folding helpers stay as they were.
  ...
  async function txpResolveByExactAlias(name) {
    ...
    // unique — the search doesn't rank exact holders first). #522's score-gap tie-break kept.
    const idn = mbmExactIdentity(arr, name, { scoreGap: 20 });
    let out = null;
    if (idn.status === 'unique') {
      const ent = await txpFetchEntity(idn.hit.id);
  ...
  async function txpResolveLabelByExactAlias(name) {
    ...
    let out = null;
    if (idn.status === 'unique') {
      const ent = await txpFetchEntity(idn.hit.id, 'label');
      if (ent && ent.gid) out = { entity: ent, via: idn.via };
    }
