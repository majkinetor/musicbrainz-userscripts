...
6. **Send to Picard using port** (off by default, port 8000) - appends `&tport=<port>` to the MusicBrainz URL *Send to Falcon* opens, hands the release to [Picard](https://picard.musicbrainz.org/) once a run finishes, by calling its `openalbum` endpoint directly ([#578](https://github.com/majkinetor/musicbrainz-userscripts/issues/578)). It fires *after* the run, so Picard reads the release with Falcon's links, ISRCs and cover already on it. The MusicBrainz URL also gains `&tport=<port>`, which renders MusicBrainz's own green tagger button as a manual fallback. Picard has to be running with **Browser integration** enabled and listening on that port — its own default is 8000; if it isn't reachable the Log says so rather than failing silently
