...
//
//
// Read-only on test.musicbrainz.org: the page only hosts Fusion's stylesheet.
...
test('a matched signal chip is filled and an unmatched one is not, in both themes', { tag: '@sandbox' }, async ({ page, inject }) => {
  ...
  for (const t of ['light', 'dark']) {
    const m = measure[t];
    check(m.hit.text >= 3.0, `${t}: a matched chip's own text is readable (${m.hit.text.toFixed(2)}:1)`);
    check(m.miss.text >= 2.2, `${t}: an unmatched chip is still legible, just recessive (${m.miss.text.toFixed(2)}:1)`);
    ...
    check(m.hit.fillAlpha > 0.12, `${t}: a matched chip is filled (alpha ${m.hit.fillAlpha.toFixed(2)})`);
    check(m.miss.fillAlpha === 0, `${t}: an unmatched chip has no fill at all (alpha ${m.miss.fillAlpha.toFixed(2)})`);
    check(m.miss.opacity === 1 && m.hit.opacity === 1, `${t}: neither is dimmed with opacity (miss ${m.miss.opacity}, hit ${m.hit.opacity})`);
  }
});
